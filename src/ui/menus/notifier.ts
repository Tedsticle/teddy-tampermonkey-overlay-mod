...
`;
  ...
  const refreshRulesUI = () => {
    ...
    for (let i = 0; i + 4 < kids.length; i += 5) {
      const itemCell = kids[i] as HTMLDivElement;
      const ruleCell = kids[i + 4] as HTMLDivElement | undefined;
  ...
  const softUpdateRenderedRows = (next: NotifierState) => {
    ...
    for (let i = 0; i + 4 < kids.length; i += 5) {
      ...
      const autobuyCell = kids[i + 3] as HTMLDivElement;
      const ruleCell = kids[i + 4] as HTMLDivElement | undefined;
      ...
      const autobuySwitch = autobuyCell.querySelector("label") as HTMLLabelElement | null;

      if (popupSwitch) setSwitchVisual(popupSwitch, !!row.popup);
      if (autobuySwitch) setSwitchVisual(autobuySwitch, !!row.autobuy);
      ...
      if (autobuySwitch) setSwitchCapState(autobuySwitch, capped);
    }
  };
  ...
  const addRow = (row: NotifierRow) => {
    ...
    // switch Autobuy (branché au service)
    const autobuySwitch = createSwitch((on) => {
      try {
        NotifierService.setAutobuy(row.id, !!on);
      } catch {}
    });

    // init switch à l’état stocké, sans déclencher setAutobuy
    setSwitchVisual(autobuySwitch, !!row.autobuy);
    (autobuySwitch as HTMLLabelElement).style.padding = "0";
    (autobuySwitch as HTMLLabelElement).title =
      "Automatically buy the remaining stock whenever this item is in the shop (the purchase still fails server-side if you can't afford it).";
    const autobuyCell = wrapCell(autobuySwitch);
    setSwitchCapState(autobuySwitch, capped);
    ...
    bodyGrid.append(itemCell, rarityCell, popupCell, autobuyCell, ruleCell);
